...
public class ProbeSenderThread extends Thread {
    ...
    public void run() {
        try {
            ...
            socket.send(new DatagramPacket(probe.getBytes(), probe.length(), address, DiscoveryConfig.WS_DISCOVERY_PORT));
        } catch (Exception e) {
            e.printStackTrace();
        }
        try {
            serverFinished.await(DiscoveryConfig.WS_DISCOVERY_TIMEOUT, TimeUnit.MILLISECONDS);
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
    }
